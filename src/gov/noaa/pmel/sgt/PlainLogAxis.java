/*
 * $Id: PlainAxis.java,v 1.1 2004/12/27 16:15:20 luca Exp $
 *
...
/**
 * Axis class for creation of standard "plain" log axes. An
 * {@link SpaceAxis example} is available demonstrating
 * <code>PlainLogAxis</code> use.
 *
 * @author Donald Denbo
 * @version $Revision: 1.1 $, $Date: 2004/12/27 16:15:20 $
 * @since 1.0
 */
public class PlainLogAxis extends PlainAxis implements Cloneable {
	 ...
	 **/
	public PlainLogAxis(String id) {
		super(id);
	}

	public Axis copy() {
		PlainLogAxis newAxis;
		try {
			newAxis = (PlainLogAxis)clone();
		} catch (CloneNotSupportedException e) {
			newAxis = new PlainLogAxis();
		}
		//
	...
	void draw(Graphics g) {
		...
		if(orientation_ == Axis.HORIZONTAL) {
			...
			for(i=0; i <= istop; i++) {
				...
				labelText = format.form(Math.pow(10, x));
...
//        xtitle = xt + LABEL_RATIO*labelHeight_;
			...
			float labelLength = 0.5f;
			for(i=0; i <= istop; i++) {
				yt = graph_.getYUtoP(y);
				labelText = format.form(Math.pow(10, y));
				label = new SGLabel("coordinate", labelText, new Point2D.Double(xt, yt));
				label.setAlign(SGLabel.CENTER, SGLabel.RIGHT);
				label.setOrientation(SGLabel.HORIZONTAL);
				label.setFont(labelFont_);
				label.setColor(labelColor_);
				label.setHeightP(labelHeight_);
			...
			if(labelPosition_ == NEGATIVE_SIDE) {
				xtitle = xt - labelLength*labelHeight_*0.6;
			} else {
				xtitle = xt + labelLength*labelHeight_*0.6;
			}
			if(title_ != null) {
