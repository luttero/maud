/*
 * $Id: GridCartesianRenderer.java,v 1.2 2006/01/19 14:45:48 luca Exp $
 *
...
public class GridCartesianRenderer extends CartesianRenderer {
  ...
  private void drawRaster(Graphics g) {
    ...
    count=0;
	  drawingRectangle.x = xp[0];
	  drawingRectangle.y = yp[ySize];
	  drawingRectangle.width = xp[xSize];
	  drawingRectangle.height = yp[0];
//	  System.out.println(xp[0] + " " + yp[0] + " " + xp[xSize] + " " + yp[ySize]);
    for(i=0; i < xSize; i++) {
      for(j=0; j < ySize; j++) {
        val = gValues[count];
        if(!Double.isNaN(val)) {
