/*
 * $Id: JPlotLayout.java,v 1.2 2006/01/19 14:45:50 luca Exp $
 *
...
public class JPlotLayout extends JGraphicLayout
  ...
  public static boolean autoIntensityResize = false;
  ...
  public JPlotLayout(int type,
                     ...
                     boolean is_key_pane,
                    int xDim, int yDim) {
    super(id, img, new Dimension(xDim,yDim));
...
//        colorKey_.setBorderStyle(ColorKey.PLAIN_LINE);
    ...
    // create axes
    //
    Font axfont = new Font("Helvetica", Font.PLAIN, 14);  // Luca from italic
    if(isXTime_) {
    ...
    if(data_.size() == 0) {
      ...
      if(plotType_ == GRID) {
        ...
        if(gridAttr.isRaster()) {
          ...
          if(autoIntensityResize && cmap instanceof TransformAccess) {
  ...
  private void adjustRange(SoTRange range) {
    if(range.isTime()) {
      ...
      if(end == st) {
        end += 30*86400000l;  // add 30 days
        st  -= 30*86400000l;  // substract 30 days
