...
public class LCLS2ConfigData {

//	public static final String INSTRUMENT_FILE_DIR = "Instrument file directory";
	public static final String DATA_FILE_DIR = "LCLS2 images directory";
	public static final String DETECTOR_PANEL_ENABLED = " enabled";

	public static final String LCLS2_PANEL_TYPE = "panel_type";
	public static final String LCLS2_DETECTORS_NUMBER = "detectors_number";
	public static final String LCLS2_COORD_X = "detector_origin_x";
	public static final String LCLS2_COORD_Y = "detector_origin_y";
	public static final String LCLS2_PANEL_WIDTH = "detector_width";
	public static final String LCLS2_PANEL_HEIGHT = "detector_height";
	public static final String LCLS2_PANEL_DARK_NUMBER = "pedestals_image_number";
	public static final String LCLS2_PANEL_DARK_ROTATION = "pedestals_image_rotation";
	public static final String LCLS2_PANEL_ROTATION = "panel_rotation";
	...
	public static String getPropertyValue(String property, String defaultvalue) {
		...
		return MaudPreferences.getPref("LCLS2Wizard." + property, defaultvalue);
	}
	...
	public static void readLCLSConfigDataFromFile(String filename) {
...
//		System.out.println(diclist[DETECTOR_NUMBER] + " " + diclist[ORIGIN_X]);
		...
		if (filename != null) {
			...
			if (detectorPanels != null)
				...
				detectorPanels = new Vector<LCLSDetectorPanel>(0, 1);

			if (detectorTypes != null)
				detectorTypes.removeAllElements();
			else
				detectorTypes = new Vector<LCLSDetectorType>(0, 1);
...
//								System.out.println("Pushback: " + ciffile.thestring);
				...
				for (int i = 0; i < detectorPanels.size(); i++) {
					LCLSDetectorPanel panel = detectorPanels.elementAt(i);
				...
				for (int i = 0; i < detectorTypes.size(); i++) {
					LCLSDetectorType type = detectorTypes.elementAt(i);
					int maxWidth = 0;
					int maxHeight = 0;
					for (int j = 0; j < type.sensors.size(); j++) {
						LCLSDetectorSensor sensor = type.sensors.elementAt(j);
	...
	public static Vector<LCLSDetectorType> detectorTypes = null;

	public static LCLSDetectorType getDetectorType(String typeName) {
		if (detectorTypes != null) {
			for (int i = 0; i < detectorTypes.size(); i++) {
				LCLSDetectorType type = detectorTypes.elementAt(i);
	...
	public static void setLoop(Vector avector, int loopItemsNumber) {
		...
		boolean head = true;
		int headIndex = -1;
		LCLSDetectorPanel panel = null;
		LCLSDetectorType type = null;
		LCLSDetectorSensor sensor = null;
...
//					System.out.println("Assigning " + diclist[index]);
				switch (index) {
					case PANEL_NAME:
						panel = new LCLSDetectorPanel(item.thestring);
...
//						System.out.println("Added panel " + item.thestring);
					...
					case PANEL_TYPE:
						if (headIndex == PANEL_TYPE) {
							...
							if (type == null) {
								type = new LCLSDetectorType(item.thestring);
						...
						if (panel != null) {
							...
							if (type != null) {
								while (type.sensors.size() < panel.detectorsNumber) {
									sensor = new LCLSDetectorSensor(panel.panelType);
					...
					case PANEL_ROTATION:
						if (panel != null) {
							...
							if (rotation > 85.0 && rotation < 95.0)
								panel.panelRotation = LCLSDetectorPanel.ROT_CW;
							else if (rotation > -95.0 && rotation < -85.0)
								panel.panelRotation = LCLSDetectorPanel.ROT_CCW;
							else if (rotation < -175.0 || rotation > 175.0)
								panel.panelRotation = LCLSDetectorPanel.ROT_180;
							else
								panel.panelRotation = LCLSDetectorPanel.ROT_NO;
						}
						avector.removeElementAt(i);
						loopitem--;
						break;
...
//							System.out.println("Before: " + type.typeName + " " + type.sensors.size());
							while (type.sensors.size() <= indexD) {
								sensor = new LCLSDetectorSensor(type.typeName);
								int number = type.sensors.size();
								sensor.detector_number = number;
								type.sensors.add(sensor);
							}
...
//						System.out.println("Sensor: " + sensor + ", r = " + item.thestring);
						if (sensor != null) {
							...
							while (rotation < -180)
								rotation += 360;
							if (rotation > 85.0 && rotation < 95.0)
								sensor.pedestal_rotation = LCLSDetectorPanel.ROT_CW;
							else if (rotation > -95.0 && rotation < -85.0)
								sensor.pedestal_rotation = LCLSDetectorPanel.ROT_CCW;
							else if (rotation < -175.0 || rotation > 175.0)
								sensor.pedestal_rotation = LCLSDetectorPanel.ROT_180;
							else
								sensor.pedestal_rotation = LCLSDetectorPanel.ROT_NO;
						}
						avector.removeElementAt(i);
						loopitem--;
						break;
