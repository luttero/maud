...
public class LoskoWizard extends Wizard {
	 ...
	 */
	public LoskoWizard(principalJFrame mainFrame, LoskoData data) {
...
//		System.exit(0);
	...
	public static void setupTheAnalysis(FilePar parameterfile, LoskoData data) {
...
//		int numberIndividualBackground = MaudPreferences.getInteger("lcls2Wizard.numberOfIndividualBackgroundParameters", 3);
			numberBackground = LoskoConfigData.getPropertyValue("numberOfGeneralBackgroundParameters", 3);
		}
...
//		System.out.println("Data panels: " + data.panels.size());
		for (int i = 0; i < data.panels.size(); i++) {
			LoskoCspad panel = data.panels.elementAt(i);
...
//				System.out.println("Sensors: " + panel.detectors.size());
				for (int j = 0; j < panel.detectors.size(); j++) {
					LoskoSensorImage sensor = panel.detectors.elementAt(j);
//					System.out.println("Sensor: " + j + " " + sensor.enabled);
					if (sensor.enabled) {
						...
						String filename = LoskoConfigData.filenameToSave;
...
//		analysis.setStoreSpectraOption(original);
	...
	public static void setupTheAnalysis(principalJFrame mainFrame, LoskoData data) {
//		int max_nbkg = LCLS2ConfigData.getPropertyValue("background_parameters_to_add", 2);

		if (data.useTemplateFile)  {
			String[] folderAndName = Misc.getFolderandName(LoskoConfigData.filenameTemplate);
...
/*		if (ImageDataPanelDescriptor.IDENTIFIER.equals(identifier.toString())) {
	...
	public static void startWizard(principalJFrame mainFrame) {
		...
		LoskoData data = new LoskoData("analysis x");
		LoskoWizard wizard = new LoskoWizard(mainFrame, data);

		WizardPanelDescriptor descriptor1 = new LoskoImageDataPanelDescriptor(data);
		wizard.registerWizardPanel(LoskoImageDataPanelDescriptor.IDENTIFIER, descriptor1);

		wizard.setCurrentPanel(LoskoImageDataPanelDescriptor.IDENTIFIER);

		wizard.frame.setVisible(true);

	}

	public static boolean integrateImage(LoskoSensorImage sensor, DataFileSet dataset, LoskoData data, String filename) {
