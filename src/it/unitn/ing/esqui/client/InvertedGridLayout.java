/*
 * @(#)InvertedGridLayout.java	1.20 99/01/22
 *
 */
...
/**
 * The <code>InvertedGridLayout</code> class is a layout manager that
 ...
 *     public void init() {
 *         setLayout(new InvertedGridLayout(3,2));
 *         add(new Button("1"));
 *         add(new Button("2"));
 *         add(new Button("3"));
 *         add(new Button("4"));
 ...
 * <p>
 * <img src="images-awt/InvertedGridLayout-1.gif"
 * ALIGN=center HSPACE=10 VSPACE=7>
 * <p>
 * When both the number of rows and the number of columns have
 * been set to non-zero values, either by a constructor or
 * by the <tt>setRows</tt> and <tt>setColumns</tt> methods, the number of
 * rows specified is ignored.  Instead, the number of
 * rows is determined from the specified number or columns
 ...
 * the number of rows affects the layout only when the
 * number of columns is set to zero.
 *
 * @version 1.00, 01/02/01
 * @author 	Leonardo Cont
 */
public class InvertedGridLayout implements LayoutManager, java.io.Serializable {
  int hgap;
  int vgap;
  int rows;
  int cols;
   ...
   * @since     JDK1.0
   */
  public InvertedGridLayout(int rows, int cols) {
    this(rows, cols, 0, 0);
  }
   ...
   * @since     JDK1.0
   */
  public InvertedGridLayout(int rows, int cols, int hgap, int vgap) {
    if ((rows == 0) && (cols == 0)) {
   ...
   * @see       InvertedGridLayout#minimumLayoutSize
...
//	Modified by Leonardo Cont, January 2001
//	norws and ncols variables were inverted.
      if (ncols > 0) {
   ...
   * grid layout.
   *
   * <p>The minimum width of a grid layout is the largest minimum width
   ...
   *
   * <p>The minimum height of a grid layout is the largest minimum height
   ...
   *                      subcomponents of the specified container.
   * @see         InvertedGridLayout#preferredLayoutSize
...
//	norws and ncols variables were inverted.
   ...
   *
   * <p>This method reshapes the components in the specified target
   * container in order to satisfy the constraints of the
   * <code>InvertedGridLayout</code> object.
   *
   * <p>The grid layout manager determines the size of individual
...
//	norws and ncols variables were inverted.
      ...
      Dimension tmpSize = parent.getSize();
      int w = tmpSize.width - (insets.left + insets.right);
      int h = tmpSize.height - (insets.top + insets.bottom);
      w = (w - (ncols - 1) * hgap) / ncols;
      h = (h - (nrows - 1) * vgap) / nrows;

//	Modified by Leonardo Cont, January 2001
//	FOR cycle was inverted.
