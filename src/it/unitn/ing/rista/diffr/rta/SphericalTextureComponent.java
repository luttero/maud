...
*/
  ...
  protected static String[] diclistc = {"_texture_spherical_component_id",
      ...
      "_texture_spherical_component_hwhm", // only in degrees
  ...
  public double HNG, HNGM, HNL, HNLM, HL1, HL2, HL3, S, B, TAU, TAUS;
  ...
  public void checkConsistencyForVersion(double version) {
    if (version == 2.99996) {
      parameterField[4].setValue(0.5 * parameterField[4].getValueD());
    }
  }
  ...
  public void PARGLP(double beta) {
    ...
    B = beta * Constants.DEGTOPI;
    double BR4 = Math.sin(B / 4.0);
    S = SGRENZ / (BR4 * BR4);
    double AMP = 1.0 / (Bessel(S, 0) - Bessel(S, 1));
    HNGM = intensity / ZGE * gauss;
    HNG = 2.0 * AMP * HNGM;
    double HGB = B;
    double HGS = S;
//    HTG1 = 1.0;
//    HTG2 = AMP;
    ...
    TAUS = r__1 * r__1;
    double TAUSS = (1. + t2) / (t2 * 8.);
    double TAUSSS = (1. - t2) / (t2 * 4.);
    HNLM = intensity / ZGE * (1.0 - gauss);
    HNL = 2.0 * HNLM * TAUSSS;
//    HTL = T;
//    HTL3 = TAU;
//    HTL4 = TAUS;
...
//    HTL2 = TAUSS;
    ...
    // only for pole figure calculation, Lorentzian part
    double TH4 = t2 * t2;
    HL1 = 1. - TH4;
    HL2 = 2. * t2;
    HL3 = 1. + TH4;

    ba = 0; // reset for fgtab
//    fgtab(HGB, HGS); // only for Gaussian part and IRED != 0
  }
...
/*  private void fgtab(double b, double s) {
    double db;
    double bn, schr, delta;
    int iom, ipunkt;
