...
public class StructureFactorStandardModel extends StructureFactorModel {
  ...
  public void computeStructureFactors_old(final Phase phase, Sample asample, DataFileSet adataset) {
	  double structureFactor, intA = 1;
	  final double cuttingThreshold = 0.0001;
	  final double thermalS = phase.getThermalStrainCached();
    final ExtinctionModel extinction = phase.getActiveExtinctionModel();
...
// l removed as in error		adataset.storeComputedStructureFactors(phase, fhkl);
	}
  ...
  public static void computeStructureFactors(final Phase phase, Sample asample, DataFileSet adataset) {
    ...
    final double thermalS = phase.getThermalStrainCached();
    final ExtinctionModel extinction = phase.getActiveExtinctionModel();
    boolean extinctionCorr = phase.getActiveExtinctionModel().canCorrect(adataset);
    phase.refreshFhklcompv();
    int hkln = phase.gethklNumber();
        ...
        if (extinctionCorr) {
          if (constantWavelength) {
            for (int i = 0; i < hkln; i++) {
              Reflection refl = phase.getReflex(i);
              for (int n = 0; n < nlines; n++) {
                fhkl[i][n] *= extinction.getExtinctionCorrectionByWave(refl.d_space, fhkl[i][n],
                    rad1.getWavelengthValue());
              }
            }
          } else {
            for (int i = 0; i < hkln; i++) {
              Reflection refl = phase.getReflex(i);
              for (int n = 0; n < nlines; n++) {
                fhkl[i][n] *= extinction.getExtinctionCorrectionByThetaRadiants(refl.d_space, fhkl[i][n],
                    thetar);
              }
            }
          }
