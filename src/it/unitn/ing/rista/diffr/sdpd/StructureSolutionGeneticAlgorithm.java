/*
 * @(#)StructureSolutionGeneticAlgorithm.java created 19/08/2001 Riva Del Garda
 *
 ...
 *
 * @version $Revision: 1.14 $, $Date: 2006/07/20 13:39:06 $
 * @author Luca Lutterotti
 * @since JDK1.1
 */

public class StructureSolutionGeneticAlgorithm extends StructureSolutionMethod
...
// ec.EvolutionState
  ...
  double defWSS = 0.0;
  double bestWSS = 0.0;
  ForceFieldSimpleRepulsion ESFField = null;
  ...
  public StructureSolutionGeneticAlgorithm(XRDcat aobj) {
    this(aobj, "Genetic Algorithm SDPD");
  }
...
//  Generate the starting structure configuration
  ...
  void initAll() {
    ...
    lbound = aphase.getStructureParamsLBound();
    ubound = aphase.getStructureParamsUBound();
...
//    ESFField = new ForceFieldSimpleRepulsion(aphase);
  }
  ...
  void startSolutionLoop() {
    ...
    String[] args = new String[argsToDefine.length + (defParams.length) * 4];
    ...
    for (int i = 0; i < defParams.length; i++) {
      ...
      args[i * 4 + 1 + actualNumber] = GeneticAlgorithmRefinement.MIN_GENE + Integer.toString(i) + "=" +
                                              Float.toString((float) lbound[i]);
      args[i * 4 + 2 + actualNumber] = "-p";
      args[i * 4 + 3 + actualNumber] = GeneticAlgorithmRefinement.MAX_GENE + Integer.toString(i) + "=" +
