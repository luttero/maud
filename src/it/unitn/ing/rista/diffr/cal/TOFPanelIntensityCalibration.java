...
public class TOFPanelIntensityCalibration extends IntensityCalibration {

	public static String[] diclistc = {"_inst_inc_parameter_file", "_instrument_counter_panel",
			"_inst_incident_spectrum_split_position",

			"_instrument_counter_panel_ID", "_inst_incident_spectrum_type",
	...
	public static String[] diclistcrm = {"_inst_inc_parameter_file", "_instrument_counter_panel",
			"_inst_incident_spectrum_split_position",
	...
	public TOFPanelIntensityCalibration(XRDcat aobj, String alabel) {
		super(aobj, alabel);
		initBaseObject();
		identifier = "TOF Incident Spectrum";
		IDlabel = "TOF Incident Spectrum";
	}
...
//	  System.out.println("Check " + getBankID());
		if (bankID.equalsIgnoreCase(getPanelID())) {
			for (int i = 0; i < banknumbers(); i++) {
				if (getPanelID(i).equalsIgnoreCase(bankID))
					return i;
			}
...
//	              ", number of functions: " + stringloopField[1].size());
								...
								if (getTypeNumber(banknumber - 1) == 10) {
									try {
										...
										System.out.println("Problem loading incident intensity file for panel number: " + banknumber + "!");
	...
	public void writeCustomObject(BufferedWriter out) {
		...
		try {
			...
			out.newLine();
			out.write("_instrument_counter_panels_number " + banknumbers() + " # not mandatory, not needed on reading");
			out.newLine();
			for (int bank = 0; bank < banknumbers() && bank < incidentSpectrum.size(); bank++) {
				out.newLine();
				out.write("_instrument_counter_panel " + bank + " # not mandatory, not needed on reading");
				out.newLine();
	...
	public void readCustomObject(CIFtoken ciffile) {
		...
		int numberOfBanks = 0;
		incidentSpectrum = new ArrayList<>(banknumbers());
...
//									System.out.println("Adding incident data, 2");
						...
						} else if (thecife.equalsIgnoreCase("_instrument_counter_panels_number")) {
...
//					System.out.println("Adding incident data, 3");
			...
			if (numberOfBanks != incidentSpectrum.size()) {
				System.out.println("Warning: number of panels does not correspond in loading incident spectra for " + toXRDcatString());
			}
...
//		System.out.println("Setting data " + incidentData.length + " " + bank);
	...
	public void removePanel(int indexToRemove) {
		removePanel(TOFPanelCalibration.panelPrefix + Integer.toString(indexToRemove));
	}
...
//		    System.out.println(this + " " + bank + " " + index + " " + incidentSpectrum.get(bank).length + " " + incidentSpectrum.size());
	...
	public JOptionsDialog getOptionsDialog(Frame parent) {
		JOptionsDialog adialog = new JPanelOptionsD(parent, this);
		return adialog;
	}

	class JPanelOptionsD extends JOptionsDialog {
...
//	  boolean removingBank = false;
		...
		public JPanelOptionsD(Frame parent, XRDcat obj) {
			...
			jp2.add(plotButton);
			plotButton.setToolTipText("Plot incident spectrum for the selected panel");
			...
			jp2.add(plotButton1);
			plotButton1.setToolTipText("Plot split incident spectrum for the selected panel");
			...
			setTitle("TOF panel intensity calibration");
			initParameters();
...
//			  removingBank = true;
				...
				if (newIndex < bankCB.getItemCount() - 1)
					...
					newIndex--;
				if (newIndex >= 0) {
					bankCB.setSelectedItem(getPanelID(newIndex));
					bankchanged();
				} else
					removeParameterFields();
