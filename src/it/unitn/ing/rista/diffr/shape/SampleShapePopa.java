/*
 * @(#)SampleShapePopa.java created 31/12/2000 Casalino
 *
 ...
 *
 *
 * @version $Revision: 1.18 $, $Date: 2006/01/19 14:45:58 $
...
public class SampleShapePopa extends SampleShape implements Shape3D {
  ...
  protected static String[] diclistc = {
    ...
    "_riet_par_sampleshape_model_weight",
  ...
  public SampleShapePopa(XRDcat aobj) {
...
//    return incidentPathLength + diffractedPathLength;
        double xdiff = incidentPathLength * Math.cos(incidentAndDiffraction_angles[i][1]) * sin0 -
                diffractedPathLength * Math.cos(incidentAndDiffraction_angles[i][3]) * sin2;
        double ydiff = incidentPathLength * Math.sin(incidentAndDiffraction_angles[i][1]) * sin0 -
                diffractedPathLength * Math.sin(incidentAndDiffraction_angles[i][3]) * sin2;
        double zdiff = incidentPathLength * Math.cos(incidentAndDiffraction_angles[i][0]) -
                diffractedPathLength * Math.cos(incidentAndDiffraction_angles[i][2]);
...
//                   + incidentAndDiffraction_angles[3] + " " + totalPath);
      if (positionCorr)
        arg1 = absorption * (totalPath * position[i] - meanShapeCorr);// * Constants.LAMBDA_SPEED_NEUTRON_CONV_REC;
      else
...
//                   + incidentAndDiffraction_angles[3] + " " + totalPath);
  ...
  public JOptionsDialog getOptionsDialog(Frame parent) {
    ...
    JOptionsDialog adialog = new JShapeHarmOptionsD(parent, this);
    return adialog;
  }
  ...
  public class JShapeHarmOptionsD extends JOptionsDialog {
    ...
    public JShapeHarmOptionsD(Frame parent, XRDcat obj) {
      ...
      showShapeB.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent event) {
          showSampleShape(JShapeHarmOptionsD.this, 1);
        }
      });
      ...
      showShapeB.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent event) {
          showSampleShape(JShapeHarmOptionsD.this, 2);
        }
      });
      ...
      showShapeB.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent event) {
          showSampleShape(JShapeHarmOptionsD.this, 3);
        }
      });
...
//			if (Constants.OpenGL)
    ...
    public void showSampleShape(Frame parent, int mode) {
      ...
      shapeFrame.getContentPane().add(sampleshape = new Show3DShape(SampleShapePopa.this, mode, value, scaleplot));
