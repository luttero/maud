/*
 * @(#)DoubleColumnQLDataFile.java created 28/03/2000 Le Mans
 *
...
public class DoubleColumnQLDataFile extends DoubleColumnQDataFile {

  public DoubleColumnQLDataFile(XRDcat aobj, String alabel) {
    super(aobj, alabel);
    identifier = ".dlq";
  }

  public DoubleColumnQLDataFile() {
    identifier = ".dlq";
  }
...
//          		token1 = st.nextToken();
        ...
        for (int i = 0; i < datanumber; i++) {
          ...
          double orX = Double.valueOf(token1).doubleValue();
          twotheta = 2 * MoreMath.asind(orX * lambda / (4 * Math.PI));
          setXData(i, twotheta);
          double intensityValue = Double.valueOf(token2).doubleValue();
          setYData(i, intensityValue);
          double tmpweight = Math.sqrt(intensityValue); //1.0; //orX * orX;
          if (tmpweight > 0.0)
