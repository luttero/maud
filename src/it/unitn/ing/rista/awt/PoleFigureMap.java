...
//    System.out.println("Pole figure " + label + ", min/max : " + amin + " " +amax);
    ...
    SGTMetaData yMeta = new SGTMetaData("", "");
    SimpleGrid sg = new SimpleGrid(values, xaxis, yaxis, label);
...
//    rpl.setSize(new Dimension(400, 400));
	  Dimension2D dim2D = new Dimension2D(6, 6);
...
//    rpl.setAxesOriginP(new Point2D.Double(0.1, 0.1));
    ...
    try {
      ...
      graph.getYAxis("Left Axis").setVisible(false);
...
/*				  Enumeration en = layer.childElements();
				  while (en.hasMoreElements()) {
					  ...
					  g.drawOval(dim.x, dim.y, dim.width - 1 - dim.x, dim.height - 1 - dim.y);
...
//    editMenu.setMnemonic('e');
  ...
  public static class WhiteMap extends JComponent {
    ...
    public WhiteMap(int width, int height) {
      this.width = width;
      this.height = height;
    }
