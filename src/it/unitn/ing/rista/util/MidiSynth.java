...
public class MidiSynth extends JPanel //implements ControlContext
{
  ...
  class Key extends Rectangle {
    ...
    public Key(int x, int y, int width, int height, int num, char ch) {
...
//      getPiano().requestFocus();
    ...
    public boolean charAssociatedIs(char ch) {
      if (ch == associatedChar)
        return true;
      return false;
    }
  } // End class Key
   ...
   * channel.
   */
  class Piano extends JPanel implements KeyListener, MouseListener {
    ...
    public Piano() {
      ...
      char[] charTable = {'Q', '@', 'W', '#', 'E', '$', 'R', 'T', '^', 'Y', '&', 'U',
                          '*', 'I', 'O', '1', 'q', 'w', '3', 'e', '4', 'r', '5', 't',
                          'y', '7', 'u', '8', 'i', 'o', '0', 'p', '-', '[', '=', ']',
      ...
      for (int i = 0, x = 0; i < 6; i++) {
        for (int j = 0; j < 7; j++, x += kw) {
          ...
          whiteKeys.add(new Key(x, 0, kw, kh, keyNum, charTable[keyNum - transpose]));
        }
      }
      for (int i = 0, x = 0; i < 6; i++, x += kw) {
        int keyNum = i * 12 + transpose;
        blackKeys.add(new Key((x += kw) - 4, 0, kw / 2, kh / 2, keyNum + 1, charTable[keyNum + 1 - transpose]));
        blackKeys.add(new Key((x += kw) - 4, 0, kw / 2, kh / 2, keyNum + 3, charTable[keyNum + 3 - transpose]));
        x += kw;
        blackKeys.add(new Key((x += kw) - 4, 0, kw / 2, kh / 2, keyNum + 6, charTable[keyNum + 6 - transpose]));
        blackKeys.add(new Key((x += kw) - 4, 0, kw / 2, kh / 2, keyNum + 8, charTable[keyNum + 8 - transpose]));
        blackKeys.add(new Key((x += kw) - 4, 0, kw / 2, kh / 2, keyNum + 10, charTable[keyNum + 10 - transpose]));
      }
    ...
    public void keyPressed(KeyEvent e) {
      ...
      Key pressedKey = getKey(ch);
      if (pressedKey != null && !pressedKey.isNoteOn()) {
        pressedKey.on();
        repaint();
      }
    }
    ...
    public Key getKey(char ch) {
      for (int i = 0; i < keys.size(); i++) {
        if (((Key) keys.get(i)).charAssociatedIs(ch)) {
          return (Key) keys.get(i);
        }
      }
      return null;
...
//                JavaSound.showInfoDialog();
  ...
  public static void main(String args[]) {
    createSynthAndShow().addWindowListener(new WindowAdapter() {
      public void windowClosing(WindowEvent e) {
        System.exit(0);
      }
    });
  ...
  public static JFrame createSynthAndShow() {
    ...
    f.setSize(w, h);
    f.setVisible(true);
    f.addKeyListener(midiSynth.getPiano());
    midiSynth.addPianoKeyListener(f);
    midiSynth.getPiano().requestFocus();

    return f;
  }

  private void addPianoKeyListener(Container f) {
    Component[] comp = f.getComponents();
    if (comp != null)
    for(int i = 0; i < comp.length; i++) {
      if (f != null) {
        comp[i].addKeyListener(getPiano());
        if (comp[i] instanceof Container)
          addPianoKeyListener((Container) comp[i]);
      }
    }
