...
/** Creates a Spectra profile plot of a rectangular selection or line selection. */
public class SpectraProfilePlot {

  static final int MIN_WIDTH = 350;
  static final double ASPECT_RATIO = 0.5;
  protected double[] profile;
  protected double[][] multiprofile;
  ...
  public SpectraProfilePlot() {
  }

  public SpectraProfilePlot(ImagePlus imp) {
    this(imp, false);
  }

  public SpectraProfilePlot(ImagePlus imp, boolean averageHorizontally) {
    Roi roi = imp.getRoi();
    if (roi == null) {
    ...
    if (!(roiType == Roi.LINE || roiType == Roi.POLYLINE || roiType == Roi.FREELINE || roiType == Roi.RECTANGLE
            || roiType == Roi.OVAL || roiType == LaueOvalStepRoi.LAUE_OVAL_STEP_ROI
            || roiType == LaueCircleRoi.CIRCLE|| roiType == LaueCircleStepRoi.LAUE_CIRCLE_STEP_ROI
            || roiType == FlatCCDReflectionSquareRoi.CCD_OVAL_STEP_ROI)) {
      IJ.error("Line or rectangular selection required.");
      return;
    }
//      System.out.println(roiType);
    ...
    if (roiType == Roi.LINE)
      ...
      multiprofile = ((LaueOvalRoi) roi).getIntervalPixels();
    else if (roiType == LaueOvalStepRoi.LAUE_OVAL_STEP_ROI)
      multiprofile = ((LaueOvalStepRoi) roi).getIntervalPixels();
    else if (roiType == FlatCCDReflectionSquareRoi.CCD_OVAL_STEP_ROI)
      multiprofile = ((FlatCCDReflectionSquareRoi) roi).getIntervalPixels();
    else if (roiType == LaueCircleRoi.CIRCLE)
	    multiprofile = ((LaueCircleRoi) roi).getPixels();
    else if (roiType == LaueCircleStepRoi.LAUE_CIRCLE_STEP_ROI)
      multiprofile = ((LaueCircleStepRoi) roi).getIntervalPixels();
  ...
  public Dimension getPlotSize() {
    if (profile == null || multiprofile == null) return null;
    int width = 0;
    if (profile != null) {
      width = (int) (profile.length * magnification);
    } else {
      width = (int) (multiprofile[0].length * magnification);
    }
  ...
  public void createWindow(ImagePlus imp) {
    if (profile != null) {
      ...
      SpectraPlotWindow pw = new SpectraPlotWindow(imp, "", xLabel, yLabel, xValues, yValues);
      if (fixedYScale) {
...
/*      Dimension d = getPlotSize();
      String xLabel = "Distance ("+units+")";
      int tot = multiprofile.length;
      int n = multiprofile[0].length;
      if (tot > n) {
        tot = tot / n;
        System.out.println("tot problem");
      }
      ...
      for (int j = 0; j < tot; j++) {
        ...
        for (int i=0; i<n; i++)
          yValues[i] = (float)multiprofile[j][i];
        SpectraPlotWindow pw = new SpectraPlotWindow("", xLabel, yLabel, xValues, yValues);
        if (fixedYScale) {
          double[] a = Tools.getMinMax(xValues);
          pw.setLimits(a[0],a[1],fixedMin,fixedMax);
        }
  ...
  /** Displays this profile plot in a window. */
  public void createWindow(ImagePlus imp, Roi usableRoi) {
    if (profile != null) {
      Dimension d = getPlotSize();
      String xLabel = "Distance (" + units + ")";
      int n = profile.length;
      ...
      for (int i = 0; i < n; i++)
        xValues[i] = (float) ((i+startX) * pixelSize);
      float[] yValues = new float[n];
  ...
  double[] getRowAverageProfile(Rectangle rect, Calibration cal, ImageProcessor ip) {
    ...
    for (int x = rect.x; x < rect.x + rect.width; x++) {
      ...
      for (int i = 0; i < rect.height; i++) {
        profile[i] += aLine[i];
        if (aLine[i] > maxGray) maxGray *= 256;
      }
    }
    for (int i = 0; i < rect.height; i++) {
      profile[i] /= rect.width;
//      if (Constants.macosx)
//        profile[i] = maxGray - profile[i];
    }
    if (cal != null)
