...
public class PlotWindow extends ImageWindow implements ActionListener, ClipboardOwner {
  ...
  public void draw() {
    ...
    buttons = new Panel();
    buttons.setLayout(new FlowLayout(FlowLayout.RIGHT));
    Label coord = new Label(" Min coord: ");
    coord.setFont(new Font("Monospaced", Font.PLAIN, 12));
    buttons.add(coord);
    minT = new TextField((new Double(0.0)).toString());
    buttons.add(minT);
    coord = new Label(" Max coord: ");
    coord.setFont(new Font("Monospaced", Font.PLAIN, 12));
    buttons.add(coord);
    maxT = new TextField((new Double(180.0)).toString());
    buttons.add(maxT);
    coord = new Label("   ");
    coord.setFont(new Font("Monospaced", Font.PLAIN, 12));
    buttons.add(coord);
  ...
  String saveAsText() {
    ...
    FileDialog fd = new FileDialog(this, "Save as CIF...", FileDialog.SAVE);
    fd.setDirectory(Misc.checkForWindowsPath(data.getFilePar().getDirectory()));

    String filename = "put a name.cif";
    String folder = "";
//		if (!Constants.windoze)
    fd.setFile(filename);
    fd.setVisible(true);
    if (fd.getFile() != null) {
      filename = Misc.filterFileName(fd.getFile());
      folder = Misc.filterFileName(fd.getDirectory());
    }
    fd.dispose();

    double xmin = Double.valueOf(minT.getText()).doubleValue();
    ...
    return Constants.startPath + Misc.filterFileName(new String(folder + filename));
