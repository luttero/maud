/*
 * Copyright 2012, Radiographema and/or its affiliates. All rights reserved.
 *
 ...
 *
 * Please contact Radiographema, 500 Radiographema Parkway, Redwood Shores, CA 94065 USA
...
#define JAVA_RUNTIME  "/Library/Java/Home"
...
int launch(char *commandName, int progargc, char *progargv[]) {
    ...
    if (workingDir != nil) {
        ...
        workingDir = NSHomeDirectory(); // REVIEW: Check which if these ones is really the users home directory ...
    }
    if (isDebugging) {
    ...
    if (runtime != nil) {
        ...
        runtimePath = findJavaDylib (jvmRequired, false, true, isDebugging, true);
        javaDylib = [runtimePath stringByAppendingPathComponent:@LIBJLI_DY_LIB];
